...
export default function LogTranPage() {
    ...
    async function fetchReportsByPage(pageNumber: number) {
        const url = `/api/admin/logs/trans?page=${pageNumber}`;
    ...
    const renderPageNumbers = () => {
        ...
        for (let i = startPage; i < endPage; i++) {
            pageNumbers.push(
                <Link
                    key={i}
                    href={`/admin/logs/trans?page=${i + 1}`}
                    onClick={(e) => {
                        if (currentPage === i) {
                            e.preventDefault();
                        } else {
    ...
    return (
        <div className="p-5">
            ...
            <div className="flex justify-center space-x-4">
                <Link
                    href={!disablePrevious ? `/admin/trans?page=${Math.floor(currentPage / maxPageButtons) * maxPageButtons - maxPageButtons + 1}` : "#"}
                    onClick={e => {
                        if (disablePrevious) {
                            e.preventDefault();
                        } else {
                ...
                {renderPageNumbers()}
                <Link
                    href={!disableNext ? `/admin/trans?page=${Math.floor(currentPage / maxPageButtons) * maxPageButtons + maxPageButtons + 1}` : "#"}
                    onClick={e => {
                        if (disableNext) {
                            e.preventDefault();
                        } else {
