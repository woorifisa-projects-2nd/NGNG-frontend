...
export default function ThirdPage({ user, setUser, currentPage, setCurrentPage }: ThirdPageProps) {
  ...
  const sendEmail = async () => {
    ...
    const url = "http://localhost:8080/join/auth/email";
    const options = {
      headers: {
  ...
  const join = async () => {
    ...
    setUser({ ...user, email: insertEmail, nickname: insertNickname, password: insertPassword });
      ...
      <div className="flex justify-center items-center h-screen">
        <div className="flex-col justify-center items-center w-5/6 md:w-1/4 h-auto">
          ...
          <div className="text-sm mt-14">
            ...
            <span> {">"} </span>
            <button type="button" onClick={() => setCurrentPage(2)}>본인확인</button>
            <span> {">"} </span>
          ...
          <div className="flex-col h-auto mt-2">
            <div className="flex justify-between items-center w-full h-14">
              <div className="flex justify-center items-center w-[75%] p-2 rounded-md border-[1px] dark:bg-[#3B3B3B] border-black/45 h-14">
                ...
                <input type="text" placeholder="이메일" value={insertEmail}
              ...
              <button className="flex justify-center items-center rounded-md text-white bg-point-color w-[20%] h-14 text-xl"
                onClick={sendEmail}>인증</button>
            </div>
            <div className={`flex justify-between items-center w-full h-14 mt-2 ${isCheckAuthNumberHidden ? "hidden" : ""}`}>
              <div className="flex justify-center items-center w-[75%] p-2 rounded-md border-[1px] dark:bg-[#3B3B3B] border-black/45 h-14">
                <AuthCheck className="w-9 mr-4 ml-2 fill-black/50 dark:fill-[#9CA3AF]" />
                <input type="text" placeholder="인증번호" value={checkAuthNumber}
              ...
              <button className="flex justify-center items-center rounded-md text-white bg-point-color w-[20%] h-14 text-xl"
                onClick={authCheck}
                disabled={isCheckAuthNumberDisabled}>확인</button>
            </div>
            <div className="flex justify-center items-center w-full p-2 rounded-md border-[1px] dark:bg-[#3B3B3B] border-black/45 h-14 mt-8">
              ...
              <input type="password" placeholder="비밀번호" value={insertPassword}
            ...
            <div className="flex justify-center items-center w-full p-2 rounded-md border-[1px] dark:bg-[#3B3B3B] border-black/45 h-14 mt-2">
              <AuthCheck className="w-9 mr-4 ml-2 fill-black/50 dark:fill-[#9CA3AF]" />
              <input type="password" placeholder="비밀번호 확인" value={checkPassword}
                onChange={e => setCheckPassword(e.target.value)}
                className="w-full focus:outline-none text-xl" />
            </div>
            <div className="flex justify-center items-center w-full p-2 rounded-md border-[1px] dark:bg-[#3B3B3B] border-black/45 h-14 mt-8">
              <UserImage className="w-9 mr-4 ml-2 fill-black/50 dark:fill-[#9CA3AF]" />
              <input type="text" placeholder="닉네임" value={insertNickname}
